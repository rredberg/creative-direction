...
def evaluate_on_split(split_name, neutral_folder, creative_folder, mdvs, thresholds):
    ...
    for layer in range(N_LAYERS):
        for token in range(1, MAX_TOKEN + 1):
            ...
            # Get activations
            test_neutral = torch.stack(a_neutral[layer][token])  # (n_samples, 4096)
            test_creative = torch.stack(a_creative[layer][token])          # (n_samples, 4096)

            # Project onto MDV
            proj_neutral = project_onto(test_neutral, mdv)  # (n_neutral,)
...
def find_best_coords_on_val(val_neutral_folder, val_creative_folder, mdvs, thresholds):
    print("\nFinding best coords on validation set...")
    val_metrics, best_coords, best_auc = evaluate_on_split("Validation", val_neutral_folder, val_creative_folder, mdvs, thresholds)
    return best_coords

def evaluate_test_given_coords(test_neutral_folder, test_creative_folder, mdvs, best_coords, thresholds):
    print("\nEvaluating on test set with best coords from validation...")
    
    layer, token = best_coords
    a_neutral = load_activations(test_neutral_folder)
    a_creative = load_activations(test_creative_folder)

    mdv = mdvs[(layer, token)]
    thresholds = threshold[(layer, token)]

    test_neutral = torch.stack(a_neutral[layer][token])  # (n_samples, hidden_dim)
...
def main():
    ...
    val_metrics, val_best_coords, val_best_auc = evaluate_on_split("Validation", val_neutral_folder, val_creative_folder, mdvs, thresholds)
